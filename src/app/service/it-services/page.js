              ...
              {Object.entries(itServiceData).map(([key, services]) =>
                services.map((service) => (
                  <TabsContent key={key} value={key}>
                    <div
                      key={service.id}
                      className="w-full  rounded-[25px] sm:rounded-[35px] flex flex-col justify-between items-start bg-white p-3 sm:p-5  lg:p-7 xl:p-8  duration-500    hover:shadow-2xl max-w-[610px] "
                    >
                      ...
                      <div className="mb-2 min-h-[50px] sm:min-h-[100px] md:min-h-[93px] lg:min-h-[140px]">
                        <p className="break-normal text-[10px] sm:text-[12px] md:text-[14px] lg:text-[18px] md:max-w-[470px] font-[600]">
                          {service.info}
                        </p>
                      </div>
                      <Link
